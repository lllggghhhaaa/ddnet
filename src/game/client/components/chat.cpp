...
using ReplacementFunction = std::function<std::string(CGameClient& client)>;
...
std::string randomPlayer(CGameClient &client)
{
	...
	for(const auto &pInfoByName : client.m_Snap.m_apInfoByName)
	{
		if(!pInfoByName)
			continue;
	...
	if(pl.size() == 0)
		pl.push_back(client.m_aClients[client.m_Snap.m_LocalClientID].m_aName);
...
std::string nearest(CGameClient &client)
{
	vec2 z = client.m_LocalCharacterPos;
	double min = std::numeric_limits<double>::max();
	std::string near = client.m_aClients[client.m_Snap.m_LocalClientID].m_aName;
